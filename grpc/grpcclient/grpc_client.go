...
func BroadcastToRoom(server *models.Server, seq string, appId uint32, userId string, cmd string, message string) (sendMsgId string, err error) {
	...
	req := protobuf.BroadcastToRoomReq{
		Msg: message,
	}
	rsp, err := c.BroadcastToRoom(ctx, &req)
	if err != nil {
